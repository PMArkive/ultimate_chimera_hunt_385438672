...
function DrawInfoBox(txt, x, y)
	...
	DrawNiceText(txt, "TargetID", x, y, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, 200);
...
function GM:HUDPaint()
	...
	if (GetState() == STATE_WAITING) then
		...
		if CurTime() > wait_time + 5 then
		
			txt = "Waiting for players...";
			
		else
		
			txt = "Starting in " .. tostring(math.Clamp(math.floor(wait_time - CurTime()), 0, wait_time));
		
		end
		
	end
	
	if (GetState() == STATE_COUNTDOWN) then
	...
	if (((ply:Alive() && ply:Team() == TEAM_PIGS) || ply:IsGhost() || ply:IsSalsa()) && !ply:IsTaunting() && !ply:IsScared()) then
		DrawCrosshair();
	end
...
function LoadWaitTime(len)
	
	wait_time = net.ReadFloat();
	
end

net.Receive("WaitTime", LoadWaitTime)
