...
if (SERVER) then
	...
	function SyncSprints()
		...
		net.Start("SyncSprints");
			for k, v in pairs(player.GetAll()) do
				
				v.UCHSprint = (v.UCHSprint || 0);
				v.UCHSprinting = (v.UCHSprinting || false);
				net.WriteTable({v, v.UCHSprint, v.UCHSprinting});
...
local function SyncSprints()
	
	local um = net.ReadTable();
	local ply = um[1];
	local sprint, sprinting = um[2], um[3];
	
	if sprint == nil || sprinting == nil || !IsValid(spint) || !IsValid(sprinting) then return end -- Sometimes will crash on spawn without this.
	
	ply:SetSprint(sprint);
	ply:SetSprinting(sprinting);
	
end
net.Receive("SyncSprints", SyncSprints);
...
function DrawSprintBar(x, y, w, h)
	...
	if ((!ply:GetSprint() && !ply:IsScared()) || (ply:Team() == TEAM_PIGS && !ply:Alive()) || ply:IsSalsa()) then
		return;
	end
