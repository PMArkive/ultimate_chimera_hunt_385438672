...
function PANEL:Init()
	...
	function self.RebuildBtn:DoClick()
		...
		CreateScoreboard();
		timer.Simple(0, function() UpdateScoreboard(self) end);
	end
...
function UpdateScoreboard(Panel)
	...
	local salsa = Panel.SalsaStatus;
	if (salsa) then
		if (!GetSalsa():IsValid()) then
			salsa:Remove();
			Panel.SalsaStatus = nil;
		else
			salsa:SetPlayer(GetSalsa());
			salsa:UpdatePlayerData();
		end
	else
		if (GetSalsa() != nil && GetSalsa():IsValid()) then
			
			Panel.SalsaStatus = vgui.Create("SalsaStatus", Panel.Frame);
			Panel.SalsaStatus:SetPlayer(GetSalsa());
